...
.PHONY: gin
## gin: runs main.go via gin
gin:
	source .env; gin --all --immediate --appPort 8088 --port 8080 run main.go
...
image-run:
	@export PATH="$$HOME/bin:$$PATH"
	docker run --rm --env-file .dockerenv -p 8080:8080 jamesclonk/${APP}:${COMMIT_SHA}
